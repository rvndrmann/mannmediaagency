  ...
  return (
    <div className="flex flex-col h-full relative">
      ...
      <div className="flex-1 overflow-hidden">
        <ScrollArea 
          ref={scrollAreaRef} 
          className={`h-full ${isMobile ? "min-h-[calc(100vh-22rem)]" : "min-h-[calc(100vh-16rem)]"}`}
        >
          <div className={`space-y-4 p-4 ${isMobile ? "pb-32" : "pb-16"}`}>
      ...
      <div className={`sticky bottom-0 left-0 right-0 w-full bg-[#1A1F2C]/95 backdrop-blur-xl border-t border-white/10 p-4 ${isMobile ? "z-40 mb-32" : "z-10"}`}>
        <ChatInput
          input={input}
          isLoading={isLoading}
