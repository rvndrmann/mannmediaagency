...
export const SplitScreen = ({
  ...
  useEffect(() => {
    if (isMobile) {
      document.body.classList.add('pb-[calc(4rem+10rem)]'); // Increased padding to accommodate chat input, AI button, and nav
      return () => {
      ...
      >
        <div 
          className={cn(
            ...
            isMobile ? (
              showChat 
                ? "fixed inset-0 z-50 animate-in fade-in slide-in pb-[calc(4rem+10rem)]" // Increased padding for all elements
                : "hidden"
            ) : (
          ...
          className={cn(
            ...
            isMobile ? (
              showChat 
                ? "hidden" 
                : cn(
                    "pb-[calc(4rem+5rem)]", // Adjusted padding for mobile view (nav + AI button)
