        ...
        <div className="space-y-3">
          <div className="p-4 bg-red-900/20 border border-red-800 rounded-lg">
            <p className="text-sm text-red-300">
              Some images failed to generate. You can try to manually check their status using the retry button.
            </p>
          </div>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {images.filter(image => image.status === 'failed').map((image, index) => (
              <div key={image.id || index} className="relative p-4 bg-gray-800/40 rounded-lg border border-red-900/30">
                <p className="text-sm text-gray-400 mb-3">
                  {image.prompt ? image.prompt.substring(0, 100) + '...' : 'Image generation failed'}
                </p>
                ...
                <Button
                  ...
                  onClick={() => onRetry && onRetry(image.id)}
                >
