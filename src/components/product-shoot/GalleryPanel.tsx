...
export function GalleryPanel({
  ...
  return (
    <div className={cn(
      "bg-[#1a1a1a]",
      isMobile ? "flex-1 min-h-0" : "flex-1 h-screen"
    )}>
      <ScrollArea className="h-full">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 p-6">
          {isLoading ? (
            ...
            <Card className="col-span-2 md:col-span-3 p-8 text-center bg-[#2a2a2a] border-gray-700">
            ...
            images.map((image) => (
              <Card 
                key={image.id}
                className="p-2 bg-[#2a2a2a] border-gray-700 animate-fade-in"
              >
                <div className="space-y-2">
                  {image.status === 'processing' ? (
                    <div className="flex flex-col items-center justify-center h-48 bg-[#222222] rounded-lg">
