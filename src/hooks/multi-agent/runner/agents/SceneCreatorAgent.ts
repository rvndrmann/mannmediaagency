...
export class SceneCreatorAgent extends BaseAgentImpl {
  ...
  async run(input: string, attachments: Attachment[]): Promise<AgentResult> {
    try {
      console.log("Running SceneCreatorAgent with input:", input, "attachments:", attachments);
      ...
      console.log("SceneCreatorAgent response:", data);
      
      // Handle handoff if present
      let nextAgent = null;
      if (data?.handoffRequest) {
        console.log(`SceneCreatorAgent handoff requested to: ${data.handoffRequest.targetAgent}`);
