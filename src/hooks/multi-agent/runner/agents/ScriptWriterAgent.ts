...
export class ScriptWriterAgent implements BaseAgent {
  ...
  async run(input: string, attachments: Attachment[]): Promise<AgentResult> {
    try {
      ...
      const { data, error } = await this.callMultiAgentFunction("script", input, attachments);
      ...
      return {
        response: data?.response || "I processed your script request but couldn't generate a response.",
