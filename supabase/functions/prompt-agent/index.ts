...
// Define the system prompt for the PromptAgent
const PROMPT_AGENT_SYSTEM_PROMPT = `**Role:** You are the PromptAgent, a specialized AI assistant within a multi-agent system designed for video creation. Your primary function is to generate detailed and effective prompts for image or video generation models (like Midjourney, Stable Diffusion, Sora, etc.) based on scene context provided by other agents.
...
try {
  ...
  // Use service role key for admin operations if needed, otherwise anon key is fine for RLS-protected tables
  // For server-side functions, service role key is often preferred for bypassing RLS when necessary.
  // Let's assume ANON key is sufficient if RLS is set up correctly for function access.
  // If updates fail, consider using the Service Role Key.
  supabase = createClient(supabaseUrl, supabaseAnonKey, {
    auth: {
      // Required for edge functions auth context
  ...
  // If Supabase fails to initialize, the function cannot proceed.
  // Consider how to handle this - maybe return an error immediately in serve?
}
...
try {
  ...
  // Similar handling if OpenAI fails to initialize
}
...
serve(async (req: Request) => {
  ...
  if (!supabase || !openai) {
      ...
      console.error('Service not available: Clients not initialized.');
  ...
  try {
    ...
    switch (action) {
      ...
      case 'call_tool': {
        ...
        const { sceneId, projectId, parameters } = body.arguments; // Extract from arguments
        ...
        const { data: sceneData, error: sceneError } = await supabase
          .from('canvas_scenes')
          .select('script, description, project_id, custom_instruction')
          .eq('id', sceneId)
          .single();
...
Scene Script:
${sceneData.script || 'No script provided.'}
...
`;
        ...
        console.log('Constructed messages for OpenAI:', JSON.stringify(messages, null, 2));
        ...
        if (!generatedPrompt) {
            const errorMessage = 'Failed to generate prompt from LLM response.';
            console.error('OpenAI response did not contain a valid prompt.');
            const errorPayload = { status: 'error', error: { message: errorMessage } };
            return new Response(JSON.stringify(errorPayload), {
                headers: { ...corsHeaders, 'Content-Type': 'application/json' },
                status: 500,
        ...
        const { error: updateError } = await supabase
          .from('canvas_scenes')
          .update({ [promptFieldToUpdate]: generatedPrompt })
          .eq('id', sceneId);
